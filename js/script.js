...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var rows = Array.from(document.querySelectorAll("#currencyTableBody tr"));
  var currentPage = 1;
  var rowsPerPage = 7;
  ...
  function displayPage(page, filteredRows) {
    ...
    var rowsToDisplay = filteredRows || rows;
  ...
  function setupPagination(filteredRows) {
    ...
    var rowsToPaginate = filteredRows || rows;
    ...
    for (var i = 1; i <= pageCount; i++) {
      ...
      pageButton.addEventListener("click", function () {
        displayPage(Number(this.innerText), rowsToPaginate);
